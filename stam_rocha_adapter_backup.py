...
def count_authors(tab_name):
    """ Returns number of authors from the author table """
    num = 0
...
def return_random_authors(tab_name, num):
    """ Returns list of all author ids and author names from the author table """
    auths = {}
    c.execute('SELECT AUTHOR_ID,AUTHOR_NM FROM {tn} ORDER BY RANDOM() LIMIT {nm}'. \
              format(tn=tab_name, nm=num))
    auths = c.fetchall()
    return auths
...
truth =[]
g_truth = {}
valid_candidates = []    # list for authors with enough tweets to meet threshold criteria
author_list = []
unknown_list = []
...
test_count = 0           # counts number of unknown messages

# Connect to the database
conn = sqlite3.connect(sqlite_file)
c = conn.cursor()
# print('Number of authors: ' + str(count_authors(table_name)))
# get candidates who have enough tweets to meet the test variable
# first, get all authors from author table
authors = return_random_authors(table_name, num_candidates*2)
# then limit the valid candidates to only those with enough tweets
valid_candidates = get_authors_over_threshold(authors, threshold, num_candidates)
    ...
    for tweet_tup in tweet_tups:
        for tweet in tweet_tup:
            ...
            # write 1 message per candidate to unknown folder for testing and ground-truth
                ...
                u_list = {}
                u_list["unknown-text"] = "unknown" + str(test_count).rjust(5, '0') + ".txt"
                unknown_list.append(u_list)
                truth_line_dict["true-author"] = candidate
                truth_line_dict["unknown-text"] = "unknown" + str(test_count).rjust(5, '0') + ".txt"
                truth.append(truth_line_dict)
                print("Tweet:" + str(threshold_check))
                print(tweet.rstrip())
                write_testing(tweet.rstrip(), test_count, candidate, os.path.join(outdir,"unknown"))
                threshold_check += 1
                test_count += 1
            else:
                break
