...
def loadJson2(corpus):
    ...
    upath += os.path.join("C:\\Users\\Robert\\CETCOE_Unknown")
    folder = os.listdir(corpusdir)
    unkfolder = os.listdir("C:\\Users\\Robert\\CETCOE_Unknown")
    encoding += "UTF8"
    language += "EN"
    for file in folder:
        candidates += file
    for file in unkfolder:
        unknowns += file
...
def getTrainingText(cand, fname):
    dfile = codecs.open(os.path.join(corpusdir, cand, fname), "r", "utf-8", "ignore")
    s = dfile.read()
    dfile.close()
    return s
...
def getUnknownText(fname):
    dfile = codecs.open(os.path.join(upath, fname), "r", "utf-8", "ignore")
    s = dfile.read()
    dfile.close()
    return s
